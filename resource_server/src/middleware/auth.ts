...
function oauth_validate(access_jwt: string, server_auth_access_token: string): Array<any>{
	...
	if (!success){ return [false, {err: 401, message: "Invalid access token"}]} //Invalid token, return 401
	...
	if (!valid){ return [false, {err: 401, message: "How??"}] }
...
router.use((req, res, next) => {
	...
	if (access_jwt == undefined){ return res.status(401).json({err: 401, message: "Access token was not provided"})}
	if (server_access_token == undefined){ return res.status(500).json({err: 500, message: "Internal authorization sever token not found."})}
	
	let [verified, message] = oauth_validate(access_jwt, server_access_token)

	if (!verified) { return res.status(message.err).json(message) }
