...
// Docstring
/**
 * Loopware Online Subsystem @ Streaming Endpoint || Simple endpoint that allows for the downloading
 * of resources from a dedicated folder
 * 
 * MISC: Might "decouple"(is that evern right) this to allow for reading from like a separate drive or some idk
 */
...
router.get("/stream", (req, res) => {
	...
	if(existsSync(filePath)){
		...
		res.status(200).download(filePath, (error) => {
			if (error){
				err(`Error while streaming data to client | ${error}`)
				return
			}
		})
		return
 ...
 * @returns void
 */
function _isValidDirectory(): boolean{
	if(existsSync(STREAMING_FOLDER_PATH)){
		return true
	}else{
		return false
	}
